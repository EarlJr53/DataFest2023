...
closed_n <- questions %>%
  separate(col = ClosedOnUtc, into = c("ClosedOn", "ignore1"), sep = -12) %>%
  select(
    StateAbbr,
    # County,
    ClosedOn,
...
asked_n <- questions %>%
  separate(col = AskedOnUtc, into = c("AskedOn", "ignore2"), sep = -12) %>%
  select(
    StateAbbr,
    # County,
    AskedOn,
...
closed_n %>%
  left_join(asked_n, by = "Date") %>%
  drop_na() %>%
  mutate(
    prop = ClosedOn / AskedOn
  ) %>%
  # filter(ClosedOn > AskedOn)
  ggplot() +
  geom_col(aes(x = Date, y = prop)) +
