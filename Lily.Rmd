...
combined_proportions %>%
  ...
  ggtitle("Proportion of Asked but Unanswered Questions for FL and TX") +
...
-   late 2018 was a huge spike for both

-   both were at a low in beginning of 2020 and have been increasing ever since

```{r}
# Need to connect the taken on and asked on values for every individual question raised. and then 

ignored_TX <- questions %>%
  filter(StateAbbr == "TX", TakenOnUtc == "NULL") %>%
  separate(col = AskedOnUtc, into = c("Date", "ignore1"), sep = -12) %>%
  select(
    Date, TakenOnUtc, -ignore1
...
# questions %>%
#   # separate(col = ClosedOnUtc, into = c("TakenTX", "ignore1"), sep = -12) %>%
#   select(
#     StateAbbr,
#     # County,
#     TakenByAttorneyUno,
